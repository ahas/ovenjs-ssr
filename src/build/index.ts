...
import { getEntryPoint, getPluginOptions, INDEX_HTML, resolveViteConfig, BuildOptions } from "../config";
import type { RollupOutput, RollupWatcher, OutputAsset, OutputOptions } from "rollup";

export default async (inlineBuildOptions: BuildOptions = {}) =>
    new Promise(async (resolve) => {
        const viteConfig = await resolveViteConfig();

        const distDir = viteConfig.build?.outDir ?? path.resolve(process.cwd(), "dist");

        const { input: inputFilePath = "", build: pluginBuildOptions = {} } = getPluginOptions(viteConfig);
        ...
        if (isWatching) {
            ...
            const clientOutputs = (Array.isArray(clientResult) ? clientResult : [clientResult as RollupOutput]).flatMap(
                (result) => result.output,
            );

            // Get the index.html from the resulting bundle.
            indexHtmlTemplate = (
                clientOutputs.find((file) => file.type === "asset" && file.fileName === INDEX_HTML) as OutputAsset
            )?.source as string;
            ...
            if (!pluginBuildOptions.keepIndexHtml) {
                await fs.unlink(path.join(clientBuildOptions.build?.outDir as string, "index.html")).catch(() => null);
            }
...
async function generatePackageJson(
    ...
    const ssrOutput = path.parse(outputFile || ((viteConfig.build?.ssr || serverBuildOptions.build?.ssr) as string));
