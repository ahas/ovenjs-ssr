...
export const createSSRDevHandler = (server: ViteDevServer, options: SsrOptions = {}) => {
    ...
    const handleSsrRequest: NextHandleFunction = async (request, response, next) => {
        ...
        try {
            ...
            const protocol =
                // @ts-ignore
                request.protocol || (request.headers.referer || "").split(":")[0] || "http";
...
export async function createSsrServer(options: InlineConfig = {}) {
...
export async function printServerInfo(server: ViteDevServer) {
    ...
    if (Object.prototype.hasOwnProperty.call(server, "printUrls")) {
        info(
            chalk.cyan(`\n  vite v${(await import("vite/package.json")).version}`) +
