...
export interface OvenSsrPluginOptions {
    /**
...
export function getPluginOptions(viteConfig: ResolvedConfig) {
    return ((viteConfig.plugins.find((plugin) => plugin.name === "@ovenjs/ssr") as any)?.ovenSsrOptions ||
        {}) as OvenSsrPluginOptions;
}
...
export async function getEntryPoint(config?: ResolvedConfig, indexHtml?: string) {
    ...
    const matches = indexHtml.substr(indexHtml.lastIndexOf('script type="module"')).match(/src="(.*)">/i);
